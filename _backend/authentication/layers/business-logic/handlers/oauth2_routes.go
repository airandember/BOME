...
func SetupOAuth2Routes(router *gin.RouterGroup, db *database.DB, oauth2Service *services.OAuth2Service) {
...
func getOAuth2Providers(c *gin.Context, oauth2Service *services.OAuth2Service) {
	providers := []OAuth2ProviderInfo{
		{
			Name:        "google",
			DisplayName: "Google",
			Icon:        "🔍",
			Enabled:     oauth2Service.IsConfigured(services.ProviderGoogle),
		},
	}
...
func initiateOAuth2Login(c *gin.Context, oauth2Service *services.OAuth2Service) {
	...
	// Validate provider
	provider := services.OAuth2Provider(req.Provider)
...
func handleOAuth2Callback(c *gin.Context, db *database.DB, oauth2Service *services.OAuth2Service) {
...
func updateOAuth2Config(c *gin.Context, db *database.DB, oauth2Service *services.OAuth2Service) {
...
func testOAuth2Provider(c *gin.Context, oauth2Service *services.OAuth2Service) {
	provider := c.Param("provider")
	if provider == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Provider parameter required"})
		return
	...
	oauth2Provider := services.OAuth2Provider(provider)
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"status": map[string]interface{}{
			"google": oauth2Service.IsConfigured(services.ProviderGoogle),
		},
	})
}
