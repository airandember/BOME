...
func GetVideosFromBunnyHandler(db *database.DB, bunnyService *videoServices.BunnyService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var videos []videoServices.BunnyVideo
		var totalItems int
		var err error
...
func GetBunnyVideoHandler(db *database.DB, bunnyService *videoServices.BunnyService) gin.HandlerFunc {
	return func(c *gin.Context) {
		videoID := c.Param("id")
...
func extractTagsFromBunnyVideo(bunnyVideo videoServices.BunnyVideo) []string {
...
func fetchBunnyVideosWithSearch(libraryID, apiKey string, search string, limit int) ([]videoServices.BunnyVideo, int, error) {
	if search == "" {
	...
	var allVideos []videoServices.BunnyVideo
	...
	var filteredVideos []videoServices.BunnyVideo
...
func fetchBunnyVideos(libraryID, apiKey string, page int, itemsPerPage int, search string) ([]videoServices.BunnyVideo, int, error) {
	// Default values
	if page < 1 {
		page = 1
	}
	...
	var response struct {
		Items        []videoServices.BunnyVideo `json:"items"`
...
func syncVideoToDatabase(db *database.DB, bunnyService *videoServices.BunnyService, bunnyVideo videoServices.BunnyVideo) error {
	description := fmt.Sprintf("Video from Bunny.net library. Duration: %d seconds, Resolution: %dx%d",
		bunnyVideo.Length, bunnyVideo.Width, bunnyVideo.Height)
	if bunnyVideo.Description != nil {
		description = *bunnyVideo.Description
	...
	_, err := videoModels.CreateVideo(db,
		bunnyVideo.Title,
		description,
		bunnyVideo.GUID,
