...
func CreateMasterVideo(db *database.DB, video *MasterVideo) (*MasterVideo, error) {
	...
	result, err := GetMasterVideoByID(db, id)
	if err != nil {
...
func GetMasterVideoByBunnyID(db *database.DB, bunnyVideoID string) (*MasterVideo, error) {
	video := &MasterVideo{}
	var tagsStr, resolutionsStr sql.NullString
	var createdBy sql.NullInt64
	var tagIDsArray pq.Int64Array
...
func GetMasterVideos(db *database.DB, limit, offset int, category, status, syncStatus, vidStatus, sortField, sortDirection string) ([]*MasterVideo, error) {
...
func DeleteMasterVideo(db *database.DB, id int) error {
	_, err := db.Exec(`DELETE FROM master_video_list WHERE id = $1`, id)
	return err
}

// GetSyncConflicts retrieves unresolved sync conflicts
func GetSyncConflicts(db *database.DB, masterVideoID *int) ([]*SyncConflict, error) {
...
func ResolveSyncConflict(db *database.DB, conflictID int, resolvedBy int, adminNotes string) error {
	_, err := db.Exec(`
...
func LogSyncAudit(db *database.DB, audit *SyncAuditLog) error {
...
func GetMasterVideoStats(db *database.DB) (map[string]interface{}, error) {
...
func SearchMasterVideos(db *database.DB, query string, limit, offset int, sortField, sortDirection string) ([]*MasterVideo, error) {
	searchQuery := `
		SELECT id, bunny_video_id, title, description, category, tags, tagged, duration, file_size,
		       resolution, framerate, thumbnail_url, video_url, iframe_src, playback_url,
		       status, views, likes, is_public, encode_progress, available_resolutions,
...
func GetMasterVideoCount(db *database.DB, category, status, syncStatus, vidStatus string) (int, error) {
...
func GetMasterVideoSearchCount(db *database.DB, query string) (int, error) {
	searchQuery := `
...
func UpdateVideoTags(db *database.DB, videoID int, tags []string) error {
	// Convert tag words to tag IDs
	tagIDs, err := convertTagWordsToIDs(db, tags)
	if err != nil {
	...
	return updateTagFrequency(db, tags)
}

// ReplaceVideoTags completely replaces tags for a video using tag IDs (used for "Tag All" functionality)
func ReplaceVideoTags(db *database.DB, videoID int, tags []string) error {
	// Convert tag words to tag IDs
	tagIDs, err := convertTagWordsToIDs(db, tags)
	if err != nil {
		return fmt.Errorf("failed to convert tag words to IDs: %v", err)
	...
	if len(oldTagIDs) > 0 {
		oldTagWords, err := convertTagIDsToWords(db, oldTagIDs)
		if err == nil && len(oldTagWords) > 0 {
			// Decrement frequency for old tags
			if err := decrementTagFrequency(db, oldTagWords); err != nil {
...
func decrementTagFrequency(db *database.DB, tags []string) error {
	...
	excludedWords, err := GetExcludedWords(db, streamingSubsiteID)
	if err != nil {
...
// updateTagFrequency updates the frequency count for tags (helper function)
func updateTagFrequency(db *database.DB, tags []string) error {
...
func GetTagAnalytics(db *database.DB) (map[string]interface{}, error) {
...
func GetUntaggedVideos(db *database.DB, limit int) ([]*MasterVideo, error) {
	rows, err := db.Query(`
...
func GetSubsiteTags(db *database.DB, subsite string) ([]map[string]interface{}, error) {
...
func GetSubsiteCategories(db *database.DB, subsite string) ([]map[string]interface{}, error) {
...
func AddSubsiteTag(db *database.DB, subsite, word string) error {
...
func DeleteSubsiteTag(db *database.DB, subsite string, tagID int) error {
...
func AddSubsiteCategory(db *database.DB, subsite, name, color, description string) error {
...
func DeleteSubsiteCategory(db *database.DB, subsite string, categoryID int) error {
...
func RemoveTagFromCategory(db *database.DB, subsite string, tagID int) error {
...
func AssignSubsiteTagToCategory(db *database.DB, subsite string, tagID, categoryID int) error {
...
func ToggleTagActiveStatus(db *database.DB, subsite string, tagID int) error {
...
func GetTagCategories(db *database.DB) ([]map[string]interface{}, error) {
...
func GetArticleExclusions(db *database.DB, subsiteID int) ([]*ArticleExclusion, error) {
	query := `
...
func AddArticleExclusion(db *database.DB, subsiteID int, word string) error {
	query := `
...
func ToggleArticleExclusion(db *database.DB, subsiteID int, word string, excluded bool) error {
	query := `
...
func RemoveArticleExclusion(db *database.DB, subsiteID int, word string) error {
...
func GetExcludedWords(db *database.DB, subsiteID int) (map[string]bool, error) {
	query := `
		SELECT word 
		FROM article_exclusions 
...
func GetSubsiteID(db *database.DB, subsiteName string) (int, error) {
	var subsiteID int
...
func ResetAllTagFrequencies(db *database.DB) error {
	var streamingSubsiteID int
	err := db.QueryRow("SELECT id FROM subsites WHERE subsite_name = 'streaming'").Scan(&streamingSubsiteID)
	if err != nil {
		return fmt.Errorf("failed to get streaming subsite ID: %v", err)
...
func convertTagWordsToIDs(db *database.DB, tagWords []string) ([]int, error) {
	if len(tagWords) == 0 {
		return []int{}, nil
	}
	...
	for _, word := range tagWords {
		...
		if err == sql.ErrNoRows {
			...
			newTag, err := contentModels.CreateTag(db, word, &streamingSubsiteID)
			if err != nil {
...
func convertTagIDsToWords(db *database.DB, tagIDs pq.Int64Array) ([]string, error) {
	if len(tagIDs) == 0 {
		return []string{}, nil
	}
...
func GetVideosByTagCategory(db *database.DB, categoryID int, page, limit int) ([]MasterVideo, int, error) {
