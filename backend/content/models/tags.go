...
func GetTags(db *database.DB) ([]Tag, error) {
	query := `
...
func GetTagByID(db *database.DB, tagID int) (*Tag, error) {
	query := `
...
func TagGetCategories(db *database.DB) ([]TagCategory, error) {
	query := `
...
func GetTagCategoryByID(db *database.DB, categoryID int) (*TagCategory, error) {
	query := `
		SELECT id, name, description, color, created_at, 
		       COALESCE(subsite_ids, '{}'), subsite_id, updated_at, COALESCE(tag_ids, '{}')
		FROM tag_categories
...
func CreateTagCategory(db *database.DB, name, color, description string, subsiteID *int) (*TagCategory, error) {
	query := `
...
func UpdateTagCategoryFields(db *database.DB, categoryID int, name, color, description string) error {
	query := `
...
func DeleteTagCategory(db *database.DB, categoryID int) error {
	tx, err := db.Begin()
	if err != nil {
...
func AddTagToCategory(db *database.DB, tagID, categoryID int) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %v", err)
	}
...
func TagRemoveFromCategory(db *database.DB, tagID, categoryID int) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %v", err)
	}
...
func RemoveTagFromAllCategories(db *database.DB, tagID int) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %v", err)
	}
...
func GetTagsByCategory(db *database.DB, categoryID int) ([]Tag, error) {
	query := `
...
func GetCategoriesForTag(db *database.DB, tagID int) ([]TagCategory, error) {
	query := `
		SELECT id, name, description, color, created_at, 
		       COALESCE(subsite_ids, '{}'), subsite_id, updated_at, COALESCE(tag_ids, '{}')
		FROM tag_categories
...
func CreateTag(db *database.DB, word string, subsiteID *int) (*Tag, error) {
	var subsiteIDs []int
	if subsiteID != nil {
		subsiteIDs = []int{*subsiteID}
	}
...
func UpdateTag(db *database.DB, tagID int, word string) error {
	query := `
		UPDATE tags 
...
func DeleteTag(db *database.DB, tagID int) error {
	// First remove from all categories
	err := RemoveTagFromAllCategories(db, tagID)
	if err != nil {
...
func GetCategoryTags(db *database.DB, categoryID int) ([]Tag, error) {
	return GetTagsByCategory(db, categoryID)
}

// GetTagsBySubsite retrieves all tags for a specific subsite
func GetTagsBySubsite(db *database.DB, subsiteID int) ([]Tag, error) {
	query := `
		SELECT id, word, frequency, subsite_id_origin, created_at, 
		       updated_at, active_tag, COALESCE(category_ids, '{}'), COALESCE(subsite_ids, '{}')
		FROM tags
...
func GetTagCategoriesBySubsite(db *database.DB, subsiteID int) ([]TagCategory, error) {
	query := `
