...
func (db *DB) GetActiveSubscriptions() (int, error) {
	var count int
...
func (db *DB) TrackAnalyticsEvent(event *AnalyticsEvent) error {
...
func (db *DB) TrackAnalyticsEventsBatch(events []*AnalyticsEvent) error {
	if len(events) == 0 {
		return nil
	}
...
func (db *DB) processAnalyticsBatch(events []*AnalyticsEvent) error {
	if len(events) == 0 {
		return nil
	}
...
func (db *DB) GetUserMetrics(userID int, startDate, endDate time.Time) ([]*UserMetrics, error) {
	query := `
...
func (db *DB) GetVideoMetrics(videoID int, startDate, endDate time.Time) ([]*VideoMetrics, error) {
	query := `
...
func (db *DB) GetSystemMetrics(startTime, endTime time.Time) ([]*SystemMetrics, error) {
	query := `
...
func (db *DB) AcknowledgeAlert(alertID int, userID int) error {
	query := `
		UPDATE alerts 
...
func (db *DB) GetPublishedVideosCount() (int, error) {
	var count int
...
func (db *DB) GetPendingVideosCount() (int, error) {
	var count int
...
func (db *DB) GetDraftVideosCount() (int, error) {
	var count int
...
func (db *DB) GetAverageVideoRating() (float64, error) {
	var avg float64
...
func (db *DB) GetTopVideoCategories(limit int) ([]map[string]interface{}, error) {
	query := `
...
func (db *DB) GetNewSubscriptionsCount(startDate, endDate time.Time) (int, error) {
	var count int
...
func (db *DB) GetRevenueForPeriod(startDate, endDate time.Time) (float64, error) {
	var revenue float64
	query := `
...
func (db *DB) CalculateMRR() (float64, error) {
	var mrr float64
	query := `
		SELECT COALESCE(SUM(amount), 0) 
...
func (db *DB) CalculateARR() (float64, error) {
	var arr float64
	query := `
		SELECT COALESCE(SUM(amount), 0) 
		FROM subscriptions s
...
func (db *DB) CalculateGrowthRate(metric string, startDate, endDate time.Time) (float64, error) {
...
func (db *DB) getTotalEventsTracked() (int, error) {
	var count int
...
func (db *DB) GetAnalyticsOverview(period string) (map[string]interface{}, error) {
	// Generate cache key
...
func (db *DB) getFromCache(key string) (interface{}, error) {
	if db.Redis == nil {
...
func (db *DB) setCache(key string, value interface{}, expiration time.Duration) {
	if db.Redis == nil {
		return
	}
...
func (db *DB) InvalidateAnalyticsCache() {
	if db.Redis == nil {
		return
	}
...
func (db *DB) GetAnalyticsEventsBySubsite(subsite string, startTime, endTime time.Time, limit, offset int) ([]*AnalyticsEvent, error) {
...
func (db *DB) GetAnalyticsEventsCount(subsite string, startTime, endTime time.Time) (int, error) {
...
func (db *DB) GetActiveUsersTrend() ([]map[string]interface{}, error) {
	query := `
		SELECT 
...
func (db *DB) GetViewAnalytics() (map[string]interface{}, error) {
...
func (db *DB) GetViewAnalyticsByPeriod(period string) (map[string]interface{}, error) {
	var interval string
	switch period {
	case "1d":
		interval = "1 day"
...
func (db *DB) GetSubscriberMetrics() (map[string]interface{}, error) {
	...
	// Get monthly revenue (simplified - would need Stripe integration for real data)
	...
	`).Scan(&monthlyRevenue)
	if err != nil {
		// If query fails, use a default calculation
		monthlyRevenue = float64(activeSubscriptions) * 9.99
	}
...
func (db *DB) GetVideoStats() (map[string]interface{}, error) {
	// Get total videos
	var totalVideos int
