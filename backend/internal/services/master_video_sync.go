...
func (s *MasterVideoSyncService) SyncFromBunny() (*SyncResult, error) {
	...
	bunnyVideos, err := s.fetchAllBunnyVideos()
	if err != nil {
	...
	// Process each video
	for _, bunnyVideo := range bunnyVideos {
		err := s.processBunnyVideo(bunnyVideo, result)
		if err != nil {
			result.Errors++
			result.ErrorDetails = append(result.ErrorDetails, fmt.Sprintf("Video %s: %v", bunnyVideo.GUID, err))
		}
	}
...
func (s *MasterVideoSyncService) fetchAllBunnyVideos() ([]BunnyVideo, error) {
	var allVideos []BunnyVideo
	page := 1
	perPage := 100

	for {
		videos, totalItems, err := fetchBunnyVideos(s.bunnyService.GetStreamLibrary(), s.bunnyService.GetStreamAPIKey(), page, perPage, "")
		if err != nil {
			return nil, err
		}
...
func (s *MasterVideoSyncService) processBunnyVideo(bunnyVideo BunnyVideo, result *SyncResult) error {
	// Check if video exists in master list
	masterVideo, err := s.db.GetMasterVideoByBunnyID(bunnyVideo.GUID)
	if err != nil {
		// Video doesn't exist, create it
		return s.createMasterVideoFromBunny(bunnyVideo, result)
	}

	// Video exists, check for conflicts and update
	return s.updateMasterVideoFromBunny(masterVideo, bunnyVideo, result)
}

func (s *MasterVideoSyncService) createMasterVideoFromBunny(bunnyVideo BunnyVideo, result *SyncResult) error {
	...
	masterVideo := &database.MasterVideo{
		...
		TotalWatchTime:       int64(bunnyVideo.TotalWatchTime),
		SyncStatus:           "synced",
		LastBunnySync:        time.Now(),
		LastMasterUpdate:     time.Now(),
		MetadataVersion:      1,
		CreatedBy:            1, // System user
	}

	_, err := s.db.CreateMasterVideo(masterVideo)
	if err != nil {
