...
// ⚠️ NO MOCK DATA - Only pulls from Stripe API
...
func NewSimpleStripeSyncService(db *database.DB, stripeService *subscriptionServices.StripeService) *SimpleStripeSyncService {
...
func (s *SimpleStripeSyncService) SyncAll(ctx context.Context) error {
	log.Println("🚀 [SIMPLE-SYNC] Starting full Stripe sync - all data")
	...
	log.Println("✅ [SIMPLE-SYNC] Complete sync finished successfully")
	return nil
}

// syncProducts syncs all products from Stripe API
// ⚠️ NO MOCK DATA - Only pulls from Stripe
func (s *SimpleStripeSyncService) syncProducts(ctx context.Context) error {
	log.Println("📦 [SIMPLE-SYNC] Syncing products from Stripe...")
	...
	for iter.Next() {
		...
		if err != nil {
			log.Printf("⚠️ [SIMPLE-SYNC] Failed to upsert product %s: %v", prod.ID, err)
			continue
		}

		count++
		if count%50 == 0 {
			log.Printf("📦 [SIMPLE-SYNC] Processed %d products", count)
		}
	}
	...
	log.Printf("✅ [SIMPLE-SYNC] Synced %d products from Stripe", count)
	return nil
}

// syncPrices syncs all prices from Stripe API
// ⚠️ NO MOCK DATA - Only pulls from Stripe
func (s *SimpleStripeSyncService) syncPrices(ctx context.Context) error {
	log.Println("💰 [SIMPLE-SYNC] Syncing prices from Stripe...")
	...
	for iter.Next() {
		...
		// Get local product ID - skip if product doesn't exist
		var productID sql.NullInt64
		err := s.db.QueryRow("SELECT id FROM stripe_products WHERE stripe_id = $1", pr.Product.ID).Scan(&productID)
		if err != nil && err != sql.ErrNoRows {
			log.Printf("⚠️ [SIMPLE-SYNC] Failed to find product %s for price %s: %v", pr.Product.ID, pr.ID, err)
			continue
		}

		// Skip this price if the product doesn't exist in our database
		if !productID.Valid {
			log.Printf("⚠️ [SIMPLE-SYNC] Skipping price %s - product %s not found", pr.ID, pr.Product.ID)
			continue
		}
		...
		if err != nil {
			log.Printf("⚠️ [SIMPLE-SYNC] Failed to upsert price %s: %v", pr.ID, err)
			continue
		}

		count++
		if count%50 == 0 {
			log.Printf("💰 [SIMPLE-SYNC] Processed %d prices", count)
		}
	}
	...
	log.Printf("✅ [SIMPLE-SYNC] Synced %d prices from Stripe", count)
	return nil
}

// syncCustomers syncs all customers from Stripe API
// ⚠️ NO MOCK DATA - Only pulls from Stripe
func (s *SimpleStripeSyncService) syncCustomers(ctx context.Context) error {
	log.Println("👥 [SIMPLE-SYNC] Syncing customers from Stripe...")
	...
	for iter.Next() {
		...
		// Simple upsert - NO MOCK DATA, pure Stripe data
		query := `
		...
		if err != nil {
			log.Printf("⚠️ [SIMPLE-SYNC] Failed to upsert customer %s: %v", cust.ID, err)
			continue
		}

		count++
		if count%50 == 0 {
			log.Printf("👥 [SIMPLE-SYNC] Processed %d customers", count)
		}
	}
	...
	log.Printf("✅ [SIMPLE-SYNC] Synced %d customers from Stripe", count)
	return nil
}

// syncSubscriptions syncs all subscriptions from Stripe API
// ⚠️ NO MOCK DATA - Only pulls from Stripe
func (s *SimpleStripeSyncService) syncSubscriptions(ctx context.Context) error {
	log.Println("📋 [SIMPLE-SYNC] Syncing subscriptions from Stripe...")

	// Sync active, canceled, and trialing subscriptions
	statuses := []string{"active", "canceled", "trialing"}
	totalCount := 0

	for _, status := range statuses {
		log.Printf("📋 [SIMPLE-SYNC] Syncing %s subscriptions...", status)
		...
		for iter.Next() {
			...
			// Get local customer ID - skip if customer doesn't exist
			var customerID sql.NullInt64
			err := s.db.QueryRow("SELECT id FROM stripe_customers WHERE stripe_id = $1", sub.Customer.ID).Scan(&customerID)
			if err != nil && err != sql.ErrNoRows {
				log.Printf("⚠️ [SIMPLE-SYNC] Failed to find customer %s for subscription %s: %v", sub.Customer.ID, sub.ID, err)
				continue
			}

			// Skip this subscription if the customer doesn't exist in our database
			if !customerID.Valid {
				log.Printf("⚠️ [SIMPLE-SYNC] Skipping subscription %s - customer %s not found", sub.ID, sub.Customer.ID)
				continue
			}
			...
			// Simple upsert - pure Stripe data
			query := `
			...
			if err != nil {
				log.Printf("⚠️ [SIMPLE-SYNC] Failed to upsert subscription %s: %v", sub.ID, err)
				continue
			}
			...
			if statusCount%25 == 0 {
				log.Printf("📋 [SIMPLE-SYNC] Processed %d %s subscriptions", statusCount, status)
			}
		}
		...
		log.Printf("✅ [SIMPLE-SYNC] Synced %d %s subscriptions", statusCount, status)
	}

	log.Printf("✅ [SIMPLE-SYNC] Synced %d total subscriptions from Stripe", totalCount)
	return nil
}
...
func (s *SimpleStripeSyncService) LinkCustomersToUsers(ctx context.Context) error {
	log.Println("🔗 [SIMPLE-SYNC] Linking Stripe customers to local users by email...")
	...
	log.Printf("✅ [SIMPLE-SYNC] Linked %d Stripe customers to local users", rowsAffected)
...
func (s *SimpleStripeSyncService) LinkSubscriptionsToUsers(ctx context.Context) error {
	log.Println("🔗 [SIMPLE-SYNC] Linking active Stripe subscriptions to users...")
	...
	rowsAffected, _ := result.RowsAffected()
	log.Printf("✅ [SIMPLE-SYNC] Linked %d active subscriptions to users (populated sub_id)", rowsAffected)
	...
	if err != nil {
		log.Printf("⚠️ [SIMPLE-SYNC] Warning: Failed to clear inactive subscription links: %v", err)
	} else {
		clearRowsAffected, _ := clearResult.RowsAffected()
		if clearRowsAffected > 0 {
			log.Printf("🧹 [SIMPLE-SYNC] Cleared %d inactive subscription links from users", clearRowsAffected)
		}
	}
