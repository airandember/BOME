...
func CreateSubscription(db *database.DB, userID int, planID *int, stripeSubscriptionID, stripePriceID, status string, currentPeriodStart, currentPeriodEnd *time.Time) (*Subscription, error) {
	var id int
...
func GetSubscriptionByID(db *database.DB, id int) (*Subscription, error) {
...
func UpdateSubscriptionStatus(db *database.DB, subscriptionID int, status string) error {
...
func CancelSubscription(db *database.DB, subscriptionID int, reason string) error {
...
func ProcessRefund(db *database.DB, subscriptionID int, amount float64, reason string) error {
...
func SoftDeleteSubscription(db *database.DB, subscriptionID int) error {
...
func GetUserSubscriptionStatus(db *database.DB, userID int) (string, error) {
...
func GetSubscriptionByStripeID(db *database.DB, stripeSubscriptionID string) (*Subscription, error) {
	subscription := &Subscription{}
	err := db.QueryRow(
		`SELECT id, user_id, plan_id, stripe_subscription_id, stripe_price_id, status, current_period_start, current_period_end, 
		        cancel_at_period_end, cancellation_reason, refund_amount, refund_reason, created_at, updated_at, deleted_at 
...
func GetUserSubscriptionHistory(db *database.DB, userID int, limit, offset int) ([]*Subscription, error) {
	rows, err := db.Query(
		`SELECT id, user_id, plan_id, stripe_subscription_id, stripe_price_id, status, current_period_start, current_period_end, 
		        cancel_at_period_end, cancellation_reason, refund_amount, refund_reason, created_at, updated_at, deleted_at 
...
func GetActiveSubscriptionsCount(db *database.DB, planID *int) (int, error) {
	var count int
	var err error
...
func GetCancelledSubscriptionsCount(db *database.DB, startDate, endDate time.Time, planID *int) (int, error) {
	var count int
	var err error
...
func GetTotalRevenue(db *database.DB, planID *int) (float64, error) {
...
func GetTotalCustomersCount(db *database.DB) (int, error) {
	var count int
	err := db.QueryRow(
