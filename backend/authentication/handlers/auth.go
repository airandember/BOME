...
func RegisterHandler(db *database.DB, emailService *email.EmailService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !crypto.RegisterRateLimiter.Allow(clientIP) {
		...
		req.Email = strings.ToLower(crypto.GetGlobalCryptoService().SanitizeString(req.Email))
		req.FirstName = crypto.GetGlobalCryptoService().SanitizeString(req.FirstName)
		req.LastName = crypto.GetGlobalCryptoService().SanitizeString(req.LastName)

		// Validate email
		if err := crypto.GetGlobalCryptoService().ValidateEmail(req.Email); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}

		// Validate names
		if err := crypto.GetGlobalCryptoService().ValidateName(req.FirstName); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid first name: " + err.Error()})
			return
		}
		if err := crypto.GetGlobalCryptoService().ValidateName(req.LastName); err != nil {
		...
		existingUser, err := authModels.GetUserByEmail(db, req.Email)
		if err == nil && existingUser != nil {
			...
			verificationToken := crypto.GetGlobalCryptoService().GenerateSecureToken()
			if err := authModels.SetVerificationToken(db, existingUser.ID, verificationToken); err != nil {
		...
		user, err := authModels.CreateUser(db, req.Email, passwordHash, req.FirstName, req.LastName, "user")
		if err != nil {
		...
		verificationToken := crypto.GetGlobalCryptoService().GenerateSecureToken()
		if err := authModels.SetVerificationToken(db, user.ID, verificationToken); err != nil {
...
func RequestVerificationHandler(db *database.DB, emailService *email.EmailService) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct {
		...
		// Get user by email
		user, err := authModels.GetUserByEmail(db, req.Email)
		if err != nil {
...
func ResendVerificationHandler(db *database.DB, emailService *email.EmailService) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct {
			Email string `json:"email" binding:"required,email"`
		}
...
func LoginHandler(db *database.DB, emailService *email.EmailService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !crypto.EnhancedLoginRateLimiter.CheckLoginAttempt(req.Email, clientIP) {
			remainingTime := crypto.EnhancedLoginRateLimiter.GetRemainingLockoutTime(req.Email)
			c.JSON(http.StatusTooManyRequests, gin.H{
		...
		if err != nil {
			if err == sql.ErrNoRows {
				...
				crypto.EnhancedLoginRateLimiter.RecordFailedAttempt(req.Email, clientIP)
				...
				if db != nil {
					...
					authModels.CreateAuditLog(db, auditLog)
				}
		...
		// Verify password
		if err := crypto.GetGlobalCryptoService().CheckPassword(user.PasswordHash, req.Password); err != nil {
			// Record failed attempt
			crypto.EnhancedLoginRateLimiter.RecordFailedAttempt(req.Email, clientIP)
		...
		crypto.EnhancedLoginRateLimiter.RecordSuccessfulAttempt(req.Email)
		...
		canCreateSession, err := authModels.CheckSessionLimit(db, user.ID, maxSessions)
		if err != nil {
		...
		if db != nil {
			...
			session, err = authModels.CreateSession(db,
				user.ID,
				tokenID,
				deviceInfo,
				clientIP,
...
func RefreshTokenHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		tokenPair, err := crypto.GetGlobalCryptoService().RefreshTokenPair(req.RefreshToken)
		if err != nil {
		...
		claims, err := crypto.GetGlobalCryptoService().ParseRefreshToken(req.RefreshToken)
		if err != nil {
		...
		if db != nil {
			_, err = authModels.GetUserByID(db, claims.UserID)
			if err != nil {
...
func ForgotPasswordHandler(db *database.DB, emailService *email.EmailService) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Rate limiting
		clientIP := crypto.GetGlobalCryptoService().GetClientIP(c.Request.RemoteAddr, c.GetHeader("X-Forwarded-For"), c.GetHeader("X-Real-IP"))
		if !crypto.PasswordRateLimiter.Allow(clientIP) {
			c.JSON(http.StatusTooManyRequests, gin.H{
		...
		resetToken := crypto.GetGlobalCryptoService().GenerateSecureToken()
		expiry := time.Now().Add(1 * time.Hour)

		if err := authModels.SetPasswordResetToken(db, user.ID, resetToken, expiry); err != nil {
...
func ResetPasswordHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Validate password
		if err := crypto.GetGlobalCryptoService().ValidatePassword(req.Password); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		...
		user, err := authModels.GetUserByResetToken(db, req.Token)
		if err != nil {
		...
		// Hash new password
		hash, err := crypto.GetGlobalCryptoService().HashPassword(req.Password)
		if err != nil {
		...
		// Update password
		if err := authModels.UpdateUserPassword(db, user.ID, hash); err != nil {
		...
		// Clear reset token
		if err := authModels.ClearPasswordResetToken(db, user.ID); err != nil {
...
func VerifyEmailLinkHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		user, err := authModels.GetUserByVerificationToken(db, req.Token)
		if err != nil {
			...
			if debugUser, debugErr := authModels.GetUserByEmail(db, "aarongusa@outlook.com"); debugErr == nil {
		...
		if err := authModels.SetUserEmailVerified(db, user.ID); err != nil {
		...
		if err := authModels.ClearVerificationToken(db, user.ID); err != nil {
		...
		if needsPasswordSetup {
			...
			setupToken := crypto.GetGlobalCryptoService().GenerateSecureToken()
			if err := authModels.SetPasswordSetupToken(db, user.ID, setupToken); err != nil {
		...
		tokenPair, err := crypto.GetGlobalCryptoService().GenerateTokenPair(user.ID, user.Email, user.Role, true) // email is verified
		if err != nil {
...
func ChangePasswordHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := crypto.GetGlobalCryptoService().ValidatePassword(req.NewPassword); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		...
		// Get current user
		user, err := authModels.GetUserByID(db, userID.(int))
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Service temporarily unavailable. Please try again later.",
			})
		...
		if err := crypto.GetGlobalCryptoService().CheckPassword(user.PasswordHash, req.CurrentPassword); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Current password is incorrect"})
			return
		}

		// Hash new password
		hash, err := crypto.GetGlobalCryptoService().HashPassword(req.NewPassword)
		if err != nil {
			log.Printf("Failed to hash password: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Service temporarily unavailable. Please try again later.",
...
func LogoutHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if userExists && emailExists {
			...
			if db != nil {
				if err := authModels.UpdateLastLogout(db, userIDInt); err != nil {
				...
				if req.AllDevices {
					if err := authModels.DeactivateAllUserSessions(db, userIDInt); err != nil {
...
func UpdateCurrentUserHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		req.FirstName = crypto.GetGlobalCryptoService().SanitizeString(req.FirstName)
		req.LastName = crypto.GetGlobalCryptoService().SanitizeString(req.LastName)
		req.Bio = crypto.GetGlobalCryptoService().SanitizeString(req.Bio)
		req.Location = crypto.GetGlobalCryptoService().SanitizeString(req.Location)
		req.Website = crypto.GetGlobalCryptoService().SanitizeString(req.Website)
		req.Phone = crypto.GetGlobalCryptoService().SanitizeString(req.Phone)
		...
		err := authModels.UpdateUserProfile(db, userID.(int), updates)
		if err != nil {
...
func SetupPasswordHandler(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		user, err := authModels.GetUserByPasswordSetupToken(db, req.Token)
		if err != nil {
		...
		if err := authModels.UpdateUserPasswordWithChange(db, user.ID, hash); err != nil {
			log.Printf("Failed to update password: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Service temporarily unavailable. Please try again later.",
			})
		...
		if err := authModels.ClearPasswordSetupToken(db, user.ID); err != nil {
