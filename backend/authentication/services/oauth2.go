...
func (s *OAuth2Service) CreateOrLinkUser(userInfo *OAuth2UserInfo) (*models.User, bool, error) {
	// Check if user already exists by email
	existingUser, err := authModels.GetUserByEmail(s.db, userInfo.Email)
	if err == nil && existingUser != nil {
		...
		if userInfo.EmailVerified && !existingUser.EmailVerified {
			err = authModels.SetUserEmailVerified(s.db, existingUser.ID)
			if err != nil {
	...
	createdUser, err := authModels.CreateUser(s.db, userInfo.Email, passwordHash, userInfo.FirstName, userInfo.LastName, "user")
	if err != nil {
	...
	if userInfo.EmailVerified {
		err = authModels.SetUserEmailVerified(s.db, createdUser.ID)
		if err != nil {
