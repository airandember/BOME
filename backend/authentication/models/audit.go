...
func GetAuditLogs(db *database.DB, filters map[string]interface{}, limit, offset int) ([]*AuditLog, error) {
	query := `
...
func GetAuditLogCount(db *database.DB, filters map[string]interface{}) (int, error) {
...
func CleanupOldAuditLogs(db *database.DB, olderThan time.Duration) error {
...
func GetSecurityMetrics(db *database.DB) (map[string]interface{}, error) {
...
func CreateAdminLog(db *database.DB, adminID *int, action, resourceType string, resourceID *int, metadata map[string]interface{}, ipAddress, userAgent string) error {
