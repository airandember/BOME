...
func generateNonce() string {
	...
	if _, err := rand.Read(bytes); err != nil {
		// Fallback to time-based nonce if security/rand fails
...
func RateLimiting() gin.HandlerFunc {
	...
	// rateLimiter := NewRateLimiter() // TODO: Implement rate limiter

	return func(c *gin.Context) {
		_ = c.ClientIP() // services.GetClientIP(c.Request.RemoteAddr, c.GetHeader("X-Forwarded-For"), c.GetHeader("X-Real-IP"))

		if false { // !rateLimiter.Allow(clientIP) {
...
func AuthRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if cryptoService == nil {
			log.Printf("Crypto service not initialized")
		...
		claims, err := cryptoService.ParseToken(token)
		if err != nil {
		...
		if err := cryptoService.ValidateTokenClaims(claims); err != nil {
...
func SubscriptionAccessRequired(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if db != nil {
			hasVideoAccess, err := videoModels.HasVideoAccess(db, userID, 0) // 0 = general access check
			if err != nil {
			...
			if !hasVideoAccess {
				log.Printf("❌ [SubscriptionAccessRequired] User %d denied video access", userID)
				c.JSON(http.StatusForbidden, gin.H{
			...
			c.Set("video_access", true)
			log.Printf("✅ [SubscriptionAccessRequired] User %d has video access", userID)
		}
...
func SubscriptionValidation(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if db != nil {
			...
			if !hasVideoAccess {
				log.Printf("❌ User %d denied video access", userID)
				c.JSON(http.StatusForbidden, gin.H{
					"error":   "Video access required",
					"code":    "VIDEO_ACCESS_REQUIRED",
					"message": "You need an active video subscription or manual access to view this content",
			...
			// Store video access info in context for later use
			c.Set("video_access", true)
			log.Printf("✅ User %d has video access", userID)
		}
	...
	return func(c *gin.Context) {
		...
		if db != nil {
			...
			if subscription.PlanID.Valid {
				plan, err := subModels.GetSubscriptionPlanByID(db, int(subscription.PlanID.Int32))
				if err == nil && plan != nil {
	...
	return func(c *gin.Context) {
		...
		if tokenID, exists := c.Get("token_id"); exists && db != nil {
			if tokenIDStr, ok := tokenID.(string); ok && tokenIDStr != "" {
				...
				go func() {
					if err := authModels.UpdateSessionActivityByTokenID(db, tokenIDStr); err != nil {
...
// Enhanced authentication logging with security considerations
func logEnhancedAuth(claims *security.Claims, c *gin.Context) {
...
func shouldLogVideoAccessDetails(claims *security.Claims, c *gin.Context) bool {
