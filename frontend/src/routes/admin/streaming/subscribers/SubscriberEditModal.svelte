<script lang="ts">
	...
	// Callback props
	export let onSave: (subscriber: Subscriber) => void = () => {};
	...
	async function loadSubscriberHistory() {
		...
		try {
			historyLoading = true;
			subscriberHistory = await StreamingSubscriberService.getSubscriberHistory(subscriber.id);
			console.log(subscriberHistory);
		} catch (error) {
	...
	async function handleSuspend() {
		...
		try {
			isSubmitting = true;
			const updatedSubscriber = await StreamingSubscriberService.suspendSubscriber(subscriber.id);
			onSave(updatedSubscriber);
			showToast('Subscriber suspended successfully', 'success');
		} catch (error) {
			console.error('Error suspending subscriber:', error);
			showToast('Failed to suspend subscriber', 'error');
		} finally {
			isSubmitting = false;
		}
	}
	...
	async function handleActivate() {
		...
		try {
			isSubmitting = true;
			const updatedSubscriber = await StreamingSubscriberService.activateSubscriber(subscriber.id);
			onSave(updatedSubscriber);
			showToast('Subscriber activated successfully', 'success');
		} catch (error) {
			console.error('Error activating subscriber:', error);
			showToast('Failed to activate subscriber', 'error');
		} finally {
			isSubmitting = false;
		}
	}
	...
	async function handleAddNote() {
		...
		try {
			...
			await StreamingSubscriberService.addAdminNote(subscriber.id, {
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="info-section">
					...
					<div class="info-grid">
						<div class="info-item">
							...
							<span class="info-value">{subscriber.first_name} {subscriber.last_name}</span>
						</div>
						<div class="info-item">
						...
						<div class="info-item">
							...
							<span class="info-value">{formatDate(subscriber.created_at)}</span>
						</div>
					</div>
				</div>
				...
				<div class="info-section">
					...
					<div class="info-grid">
						<div class="info-item">
							...
							<span class="info-value">{getPlanName(subscriber.plan_id)}</span>
						</div>
						<div class="info-item">
							<label class="info-label">Subscription Status</label>
							<span class="info-value">
								<span class="status-badge {getStatusBadgeClass(subscriber.subscription_status || 'inactive')}">
									{subscriber.subscription_status || 'Inactive'}
								</span>
							</span>
						</div>
						{#if subscriber.plan_price}
							<div class="info-item">
								<label class="info-label">Plan Price</label>
								<span class="info-value">${subscriber.plan_price} {subscriber.plan_currency || 'USD'}</span>
							</div>
						{/if}
						{#if subscriber.current_period_start}
							<div class="info-item">
								<label class="info-label">Current Period Start</label>
								<span class="info-value">{formatDate(subscriber.current_period_start)}</span>
							</div>
						{/if}
						{#if subscriber.current_period_end}
							<div class="info-item">
								<label class="info-label">Current Period End</label>
								<span class="info-value">{formatDate(subscriber.current_period_end)}</span>
							</div>
						{/if}
					</div>
				</div>
				<div class="history-container">
					...
					<div class="info-section">
						<h3 class="section-title">Subscriber History</h3>
						{#if historyLoading}
