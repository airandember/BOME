<script lang="ts">
	...
	async function loadSchedulerStatus() {
		try {
			const response = await fetch('/api/v1/search-index/scheduler/status', {
				headers: {
	...
	async function loadConfiguration() {
		try {
			const response = await fetch('/api/v1/search-index/config', {
				headers: {
					'Authorization': `Bearer ${getAuthToken()}`
				}
			});
	...
	async function loadStats() {
		try {
			const response = await fetch('/api/v1/search-index/stats', {
				headers: {
					'Authorization': `Bearer ${getAuthToken()}`
				}
			});
	...
	async function triggerManualGeneration() {
		isGenerating = true;
		try {
			const response = await fetch('/api/v1/search-index/scheduler/trigger', {
				method: 'POST',
				headers: {
					'Authorization': `Bearer ${getAuthToken()}`
				}
	...
	async function downloadSearchIndex() {
		try {
			const response = await fetch('/api/v1/search-index/download', {
				headers: {
					'Authorization': `Bearer ${getAuthToken()}`
				}
			});
